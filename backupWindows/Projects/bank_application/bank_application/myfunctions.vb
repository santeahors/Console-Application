...
Module myfunctions
    ...
    Function connection()
        Dim conn As New MySqlConnection("server=localhost;userid=root;password=;database=bank_santehors")
        Return conn
    End Function
    ...
    Function getCustomerProfile(ByVal customer_id As String)
        ...
        Dim arrimage() As Byte = reader("passport")
        connection.close()
        Return Tuple.Create(fullname, last_login, account_balance, loan_balance, arrimage)
    End Function
    ...
    Public Sub customerLoanSchedule()
        Try
            Dim connection As MySql.Data.MySqlClient.MySqlConnection = myfunctions.connection
            ...
            command.Parameters.AddWithValue("@customer_id", view_loan_form.profile_id.SelectedValue)
            Dim adapter As New MySqlDataAdapter(command)
            Dim table As New DataTable()
            connection.Open()
            adapter.Fill(table)
            ...
            loan_schedule_form.loanScheduleRecord.Items.Clear()
            Dim sn As Integer = 1
            For Each row As DataRow In table.Rows
                Dim listItem As New ListViewItem((sn.ToString))
                ...
                loan_schedule_form.loanScheduleRecord.Items.Add(listItem)
                sn = sn + 1
            Next
        Catch ex As Exception
            MsgBox(ex.ToString)
    ...
    Function system_settings()
        ...
        reader = command.ExecuteReader
        reader.Read()
        Dim minimum_balance As Double = reader("minimum_balance")
        Dim maximum_loan As Double = reader("maximum_loan")
        Dim minimum_loan As Double = reader("minimum_loan")
        Dim maximum_duration As Integer = reader("maximum_duration")
        Dim loan_rate As Double = reader("loan_rate")
        connection.close()
